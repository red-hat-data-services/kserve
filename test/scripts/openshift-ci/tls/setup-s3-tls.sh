...
S3_NAMESPACE="${S3_NAMESPACE:-${NS:-kserve}}"
S3_TEST_NAMESPACE="${S3_TEST_NAMESPACE:-kserve-ci-e2e-test}"
...
# Create the S3 namespace if it does not already exist
if oc get namespace "$S3_NAMESPACE" > /dev/null 2>&1; then
    echo "Namespace $S3_NAMESPACE exists."
else
    cat <<EOF | oc apply -f -
apiVersion: v1
kind: Namespace
...
if oc get deployment $DEPLOYMENT_NAME -n "$S3_NAMESPACE" > /dev/null 2>&1; then
    echo "Deleting existing $DEPLOYMENT_NAME deployment"
    oc delete deployment $DEPLOYMENT_NAME -n "$S3_NAMESPACE" --ignore-not-found || true
fi
# Delete TLS secret if it exists (only for custom cert - serving cert is managed by OpenShift)
if [[ "$CERT_TYPE" == "custom" ]]; then
    if oc get secret $SECRET_NAME -n "$S3_NAMESPACE" > /dev/null 2>&1; then
        echo "Deleting existing $SECRET_NAME secret"
        oc delete secret $SECRET_NAME -n "$S3_NAMESPACE" --ignore-not-found || true
    fi
fi
# Clean up storage-config entry for this certificate type (will be recreated)
if oc get secret storage-config -n "$S3_TEST_NAMESPACE" > /dev/null 2>&1; then
    oc patch secret storage-config -n "$S3_TEST_NAMESPACE" --type=json \
...
kustomize build $PROJECT_ROOT/test/overlays/openshift-ci/$KUSTOMIZE_PATH |
  oc apply -n "$S3_NAMESPACE" --server-side=true -f -

# Wait for SeaweedFS deployment to be ready
echo "Waiting for $DEPLOYMENT_NAME deployment to be ready..."
oc rollout status deployment/$DEPLOYMENT_NAME -n "$S3_NAMESPACE" --timeout=300s
...
if [[ "$CERT_TYPE" == "serving" ]]; then
    ...
    (oc get secret $SECRET_NAME -n "$S3_NAMESPACE" -o jsonpath="{.data['tls\.crt']}" | base64 -d) > $TLS_DIR/certs/$CERT_DIR/tls.crt
    (oc get secret $SECRET_NAME -n "$S3_NAMESPACE" -o jsonpath="{.data['tls\.key']}" | base64 -d) > $TLS_DIR/certs/$CERT_DIR/tls.key
    ...
    oc create secret generic $SECRET_NAME --from-file=${TLS_DIR}/certs/custom/root.crt  --from-file=${TLS_DIR}/certs/custom/custom.crt --from-file=${TLS_DIR}/certs/custom/custom.key -n "$S3_NAMESPACE"
    # Patch deployment to mount certificates and enable TLS
    oc patch deployment $DEPLOYMENT_NAME -n "$S3_NAMESPACE" -p "{\"spec\":{\"template\":{\"spec\":{\"containers\":[{\"name\":\"$DEPLOYMENT_NAME\",\"args\":[\"mini\",\"-dir=/data\",\"-s3\",\"-s3.cert.file=/certs/custom.crt\",\"-s3.key.file=/certs/custom.key\"],\"volumeMounts\":[{\"mountPath\":\"/certs\",\"name\":\"$SECRET_NAME\",\"readOnly\":true},{\"mountPath\":\"/data\",\"name\":\"data\"}]}], \"volumes\":[{\"name\":\"$SECRET_NAME\",\"projected\":{\"defaultMode\":420,\"sources\":[{\"secret\":{\"name\":\"$SECRET_NAME\",\"items\":[{\"key\":\"custom.crt\",\"path\":\"custom.crt\"},{\"key\":\"custom.key\", \"path\":\"custom.key\"},{\"key\":\"root.crt\",\"path\":\"root.crt\"}]}}]}}]}}}}"
...
oc delete job "${S3_INIT_JOB_NAME}" -n "$S3_NAMESPACE" --ignore-not-found
sed -e "s/name: s3-init/name: ${S3_INIT_JOB_NAME}/" \
    -e "s|http://s3-service.kserve:8333|https://${SERVICE_NAME}.${S3_NAMESPACE}.svc:8333|" \
  ...
  oc apply -n "$S3_NAMESPACE" -f -

echo "Waiting for S3 TLS init job to complete..."
if ! oc wait --for=condition=complete "job/${S3_INIT_JOB_NAME}" -n "$S3_NAMESPACE" --timeout=300s; then
  echo "S3 TLS init job failed. Pod status and logs:"
  oc get pods -l "job-name=${S3_INIT_JOB_NAME}" -n "$S3_NAMESPACE"
  oc logs -l "job-name=${S3_INIT_JOB_NAME}" -n "$S3_NAMESPACE" --tail=50 || true
  exit 1
fi
