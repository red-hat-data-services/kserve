...
func TestInjectImageVolume(t *testing.T) {
	...
	// Test when srcURI starts with OciURIPrefix
	{
		...
		expectedVolName := utils.GetVolumeNameFromPath(constants.DefaultModelLocalMountPath)
		if len(pod.Spec.Volumes) != 1 || pod.Spec.Volumes[0].Name != expectedVolName {
			t.Errorf("Expected one volume with name %s, but got %v", expectedVolName, pod.Spec.Volumes)
		}
		...
		if len(pod.Spec.Containers[0].VolumeMounts) != 1 {
			...
			if mount.Name != expectedVolName {
				t.Errorf("Expected mount name %s but got %s", expectedVolName, mount.Name)
			}
...
func TestInjectImageVolumeMultiNode(t *testing.T) {
	...
	t.Run("Test InjectImageVolume with worker-container only (multi-node scenario)", func(t *testing.T) {
		...
		assert.Len(t, pod.Spec.Volumes, 1, "Should have exactly one volume")
		assert.Equal(t, expectedVolName, pod.Spec.Volumes[0].Name)
